...
# Dynamixel ID 정의 (사용자 코드와 동일하게 유지)
# DynamixelControlNode __init__에 따라 매핑 가정:
# self.base (ID 1, XM)
# self.joint1 (ID 2, XM)
# self.joint2 (ID 3, AX)
# self.joint3 (ID 4, AX)
# self.end_effector (ID 5, XC)
...
ADDR_PROFILE_VELOCITY = 112 # XM/XC용
...
def inverse_kinematics(target_pose_matrix, initial_guess=np.zeros(num_joints)):
    ...
    def objective_function(joints):
        ...
        # 위치 오차와 방향 오차를 결합. 방향 오차에 가중치를 부여하여 중요도를 조절할 수 있습니다.
        # 가중치가 높을수록 방향 일치에 더 큰 우선순위를 둡니다.
...
class DynamixelControlNode(Node):
    ...
    def move_to_xyz_rpy(self, x, y, z, roll_deg, pitch_deg, yaw_deg, mode='trajectory'):
        """
        ...
        if mode == 'trajectory':
            ...
            for t in range(len(time_points)):
                ...
                self.base.move(motor_angles[0])
                self.joint1.move(motor_angles[1])
                self.joint2.move(motor_angles[2])
                self.joint3.move(motor_angles[3])
                time.sleep(0.1)
    ...
    def process(self):
        ...
        # 목표 위치 및 그리퍼 방향 설정
        # (Roll=0°, Pitch=0°, Yaw=90°)는 엔드 이펙터의 Z축이 월드 좌표계의 음수 Z축 방향(수직 아래)을 향하게 합니다.
        gripper_orientation_r = 0
        gripper_orientation_p = 0
        gripper_orientation_y = 90

        # 1. 물체 위로 이동 (Pick Above)
        self.get_logger().info("--- Moving to pick-above position ---")
        # 실제 박스 위치: x값에 -0.095 추가
        pick_x = -0.095 - 0.14 
        pick_y = 0.0          
        # self.move_to_xyz_rpy(pick_x, pick_y, pick_z_above,
        #                      gripper_orientation_r, gripper_orientation_p, gripper_orientation_y)
        
        # 2. 그리퍼 열기 (고정된 값으로 제어)
        self.get_logger().info("\n--- Opening gripper (fixed value) ---")
        self.end_effector.move(270) # 예시: 그리퍼 열림을 나타내는 값
        time.sleep(1.0)
        ...
        # 4. 그리퍼 닫기 (고정된 값으로 제어)
        self.get_logger().info("\n--- Closing gripper (fixed value) ---")
        self.end_effector.move(355) # 예시: 그리퍼 닫힘을 나타내는 값
        time.sleep(1.0)
        ...
        self.get_logger().info("\n--- Moving to grasp position ---")
        pick_z_grasp = 0.04 + 0.01# 물체를 잡을 Z 좌표 (물체 높이에 맞춤)
        self.move_to_xyz_rpy(pick_x + 0.025, pick_y, pick_z_grasp-0.025, 
                             gripper_orientation_r, gripper_orientation_p, gripper_orientation_y)

        # self.move_to_xyz_rpy(pick_x + 0.025, pick_y, pick_z_grasp-0.025 + 0.005, 
        #                      gripper_orientation_r, gripper_orientation_p, gripper_orientation_y, 'direct')
        # self.move_to_xyz_rpy(pick_x + 0.025, pick_y, pick_z_grasp-0.025 + 0.01, 
        #                      gripper_orientation_r, gripper_orientation_p, gripper_orientation_y, 'direct')                                                         
        # self.move_to_xyz_rpy(pick_x + 0.025, pick_y, pick_z_grasp-0.025 + 0.02, 
        #                      gripper_orientation_r, gripper_orientation_p, gripper_orientation_y, 'direct')
        self.move_to_xyz_rpy(pick_x + 0.025, pick_y, pick_z_grasp-0.025 + 0.03, 
...
# import time
# from scipy.optimize import minimize
# from scipy.interpolate import CubicSpline  # CubicSpline 추가
...
#         self.joint1 = DynamixelMotor(self.port_handler, 2, "joint1", 1000000,  XM_PROTOCOL_VERSION, self.get_logger())  # joint1
#         self.joint2 = DynamixelMotor(self.port_handler, 3, "joint2", 1000000,  AX_PROTOCOL_VERSION, self.get_logger(),is_ax=True)  # joint2
...
#         self.base.move(212)  # base
#         self.joint1.move(182)  # joint1
#         self.joint2.move(150)  # joint2
#         self.joint3.move(240)  # joint3
#         self.end_effector.move(270)  # end-effector

#     def cubic_spline_trajectory(self, joint_angles_initial, joint_angles_final, time_points):
#         # Cubic spline을 사용하여 각 조인트의 목표 각도를 계산
#         spline_trajectory = []
#         for initial, final in zip(joint_angles_initial, joint_angles_final):
#             times = [0, max(time_points)]  # 시간 0과 목표시간
#             angles = [initial, final]
...
#         # 각도(deg)로 변환 후 Cubic spline을 통해 부드럽게 이동
#         time_points = np.linspace(0, 5, 10)  # 5초 동안 50개의 시간 포인트
#         joint_angles_initial = [0, 0, 0, 0, 0]
#         joint_angles_final = joint_angles_deg
#         spline_trajectory = self.cubic_spline_trajectory(joint_angles_initial, joint_angles_final, time_points)
        
#         for t in range(len(time_points)):
#             self.base.move(210-spline_trajectory[1][t])
#             self.joint1.move(182 - spline_trajectory[2][t])
#             self.joint2.move(150 + spline_trajectory[3][t])
#             self.joint3.move(240 + spline_trajectory[4][t])
#             time.sleep(0.1)  # 0.1초마다 이동
...
#         start_point_x = -0.095 - 0.16
#         start_point_y = 0
#         start_point_z = 0.04 + 0.01
#         target_pos = np.array([-0.095 -0.16, 0, 0.04 + 0.01])
...
#         time.sleep(1.0)
#         self.move_to_initial_position_again()
#         self.target_position(start_point_x+0.025, start_point_y, start_point_z-0.025)
